...
func TestResolveSecret(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "env var when no direct value or file",
			value:     "",
			envVar:    "TEST_ENV",
			filePath:  "",
			envValue:  "env-secret",
		...
		{
			name:      "error when env var is specified but empty",
			value:     "",
		...
		{
			name:      "direct value overrides file and env",
			value:     "direct-secret",
			envVar:    "TEST_ENV",
			filePath:  "", // will be set to temp file
			envValue:  "env-secret",
		...
		{
			name:      "file overrides env when both present",
			value:     "",
			envVar:    "TEST_ENV",
			filePath:  "", // will be set to temp file
			envValue:  "env-secret",
...
func TestResolveSecretWithFallback(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "fallback env is used when file error occurs",
			value:         "",
			envVar:        "",
		...
		{
			name:          "fallback env is used when primary env var is empty",
			value:         "",
			envVar:        "EMPTY_PRIMARY_ENV",
			filePath:      "",
			fallbackEnv:   "FALLBACK_ENV",
			envValue:      "",
			fileValue:     "",
		...
		{
			name:          "file takes precedence over env and fallback",
			value:         "",
			envVar:        "PRIMARY_ENV",
			filePath:      "", // Will be set to actual temp file path
			fallbackEnv:   "FALLBACK_ENV",
